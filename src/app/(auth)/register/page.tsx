    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input id="password" type="password" label="Password" placeholder="Create a strong password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} />
        ...
        <div className="text-center mt-6">
          <p className="text-sm text-zinc-500">Already have an account?{' '}
            <Link href={ROUTES.LOGIN} className="text-jagflix-400 hover:text-jagflix-300 font-medium">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
